...
void mydefrag(void **ptrlist[])
{
    ...
    while (currentSegment->next != NULL)
    {
        ...
        ///compress 2 empty segments in one segment
        else if (currentSegment->allocated == FALSE && currentSegment->next->allocated == FALSE)
        {
            aux = currentSegment->next;
            currentSegment->size = currentSegment->next->size + currentSegment->size;
